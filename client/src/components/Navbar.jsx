...
import {
    ...
    FiBell, FiAlertCircle, FiCheckCircle, FiCalendar
} from 'react-icons/fi';
...
const Navbar = () => {
    ...
    const getRoleLinks = () => {
        ...
        switch (user.role) {
            case 'rider':
                ...
                const links = [
                    { path: '/rider/home', label: 'Book Ride', icon: <FiMapPin /> },
                    { path: '/rider/scheduled', label: 'Scheduled', icon: <FiCalendar /> },
    ...
    return (
        <>
            <nav className={`fixed top-0 w-full z-[1001] transition-all duration-300 bg-white dark:bg-gray-900 ${scrolled ? 'shadow-md border-b border-gray-100 dark:border-gray-800 dark:shadow-gray-900/50' : ''
                }`}>
                    ...
                    <div className="flex justify-between items-center h-16">
                        ...
                        <Link to="/" className="flex items-center gap-2.5 group">
                            ...
                            <div className="flex flex-col">
                                <span className="text-lg font-bold text-gray-900 dark:text-white leading-tight">Safro</span>
                        ...
                        <div className="hidden lg:flex items-center gap-3">
                            ...
                            {user && (
                                <div className="relative">
                                    <button
                                        ...
                                        className="p-2 text-gray-500 hover:text-gray-900 hover:bg-gray-50 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800 rounded-lg transition-all relative"
                                    >
                                        <FiBell size={20} />
                                        {unreadCount > 0 && (
                                    ...
                                    <AnimatePresence>
                                        {notificationsOpen && (
                                            <motion.div
                                                ...
                                                exit={{ opacity: 0, y: 8 }}
                                                className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl dark:shadow-2xl dark:shadow-black/30 z-50 overflow-hidden"
                                            >
                                                <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
                                                    ...
                                                    <button onClick={clearNotifications} className="text-[10px] text-gray-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 font-medium">Clear all</button>
                                                </div>
                                                <div className="max-h-[350px] overflow-y-auto">
                                                    {notifications.length === 0 ? (
                                                        <div className="py-12 text-center">
                                                            <FiBell className="mx-auto text-gray-200 dark:text-gray-600 mb-2" size={32} />
                                                            <p className="text-xs text-gray-400 dark:text-gray-500">No new alerts</p>
                                                        </div>
                                                    ) : (
                                                        notifications.map((notif) => (
                                                            <Link
                                                                ...
                                                                className="flex gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors last:border-0"
                                                            >
                                                                <div className="w-8 h-8 rounded-full bg-gray-50 dark:bg-gray-700 flex items-center justify-center flex-shrink-0 mt-0.5">
                                                                    {notif.icon}
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                    <p className="text-xs font-bold text-gray-900 dark:text-white leading-tight mb-0.5">{notif.title}</p>
                                                                    <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-snug truncate">{notif.message}</p>
                            ...
                            {user ? (
                                <div className="relative">
                                    ...
                                    >
                                        <div className="w-8 h-8 bg-teal-100 dark:bg-teal-900/50 text-teal-700 dark:text-teal-400 rounded-full flex items-center justify-center text-sm font-bold">
                                            {user.name?.charAt(0)?.toUpperCase()}
                                        </div>
                                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{user.name?.split(' ')[0]}</span>
                                        <FiChevronDown className={`text-gray-400 dark:text-gray-500 transition-transform ${profileOpen ? 'rotate-180' : ''}`} />
                                    </button>
                                    ...
                                    <AnimatePresence>
                                        {profileOpen && (
                                            <motion.div
                                                ...
                                                animate={{ opacity: 1, y: 0 }}
                                                exit={{ opacity: 0, y: 8 }}
                                                className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg dark:shadow-2xl dark:shadow-black/30 py-2 z-50"
                                            >
                                                <div className="px-4 py-2 border-b border-gray-100 dark:border-gray-700">
                                                    <p className="text-sm font-semibold text-gray-900 dark:text-white">{user.name}</p>
                                                    <p className="text-xs text-gray-400 dark:text-gray-500 capitalize">{user.role}</p>
                                                </div>
                                                <Link to={`/${user.role}/profile`} className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white">
                                                    <FiUser size={14} /> Profile
                                                </Link>
                                                {user.role === 'rider' && (
                                                    <Link to="/rider/history" className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white">
                                                        <FiList size={14} /> My Rides
                                                    </Link>
                                                )}
                                                <div className="h-px bg-gray-100 dark:bg-gray-700 my-1 mx-2" />
                                                <button
                                                    onClick={handleLogout}
                                                    className="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 text-left transition-colors"
                                                >
                        ...
                        <div className="flex lg:hidden items-center gap-2">
                            ...
                            {/* Mobile Toggle */}
                            <button
                                className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-50 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800 rounded-lg transition-colors"
                                onClick={() => {
            ...
            <AnimatePresence>
                {notificationsOpen && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className="fixed inset-x-0 top-16 z-[1000] bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-lg dark:shadow-2xl dark:shadow-black/30 lg:hidden"
                    >
                        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
                            <h3 className="text-sm font-bold text-gray-900 dark:text-white">Notifications</h3>
                            <button onClick={clearNotifications} className="text-[10px] text-gray-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 font-medium">Clear all</button>
                    ...
                    >
                        <div className="px-4 py-4 space-y-1">
                            ...
                            <div className="border-t border-gray-100 dark:border-gray-700 pt-3 mt-3 space-y-1">
                                {user ? (
                                    <>
                                        <Link to={`/${user.role}/profile`} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                            <FiUser /> Profile
                                        </Link>
                                        {user.role === 'rider' && (
                                            <Link to="/rider/history" className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                                <FiList /> My Rides
                                            </Link>
                                        )}
                                        <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 rounded-lg text-sm font-medium text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20">
                                            <FiLogOut /> Logout
                                        </button>
                                    </>
                                ) : (
                                    <>
                                        <Link to="/login" className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                                            Login
                                        </Link>
