...
const colorMap = {
    teal: { base: '#0d9488', ring: 'rgba(13,148,136,0.1)' },
    red: { base: '#ef4444', ring: 'rgba(239,68,68,0.1)' },
    blue: { base: '#3b82f6', ring: 'rgba(59,130,246,0.1)' },
};
        ...
        <div className="relative w-full" ref={wrapperRef} onKeyDown={handleKeyDown}>
            <div className="relative group">
                <div 
                    ...
                    style={{ color: isOpen ? colors.base : (icon ? colors.base : '#9ca3af') }}
                >
                ...
                <input
                    ...
                    className="w-full pl-11 pr-10 py-3.5 bg-gray-50 dark:bg-gray-950/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-2xl text-sm transition-all duration-300 outline-none focus:bg-white focus:ring-4 group-hover:bg-white text-gray-800 dark:text-gray-200 font-medium"
                    style={{ '--tw-ring-color': colors.ring, borderColor: isOpen ? colors.base : undefined }}
                />

                <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    {(loading || selecting) ? (
                        <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: colors.base, borderTopColor: 'transparent' }} />
                    ) : query && (
                        <button 
                            type="button" 
                            onClick={clearInput}
                            className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300"
                        >
                            <FiX size={16} />
                        </button>
                    )}
            ...
            <AnimatePresence>
                {isOpen && (suggestions.length > 0 || error) && (
                    <motion.div
                        ...
                        className="absolute z-[100] w-full mt-2 bg-white dark:bg-gray-900/95 backdrop-blur-md rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden"
                    >
                            ...
                            {error ? (
                                <div className="p-6 flex flex-col items-center text-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Try typing manually or checking your connection.</p>
                                    </div>
                                </div>
                            ) : (
                                <ul className="pb-2">
                                        ...
                                        <li key={loc.id || index}>
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-3 flex items-start gap-4 transition-all duration-200 ${
                                                    activeIndex === index ? 'bg-gray-50 dark:bg-gray-950' : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'
                                                }`}
                                            >
                                                <div className={`mt-0.5 p-2 rounded-xl flex-shrink-0 ${
                                                    loc.isCurrentLocation ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-gray-50 dark:bg-gray-950 text-gray-400 dark:text-gray-500'
                                                }`}>
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {loc.isCurrentLocation && (
                                                            <span className="text-[10px] font-bold uppercase tracking-wider text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded-md">Nearby</span>
                                                        )}
                                                    </div>
